...
def upload_backup_bytes_to_s3(
    bundle_bytes: bytes,
    filename: str,
    bucket: str | None = None,
    base_path: str = 'scanoil-backups',
    content_type: str | None = None,
):
    """Upload backup to S3-compatible storage: AWS S3, Cloudflare R2, Yandex Object Storage, etc.

    Required env when used from script:
    BACKUP_S3_BUCKET, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY.
    Optional: BACKUP_S3_ENDPOINT_URL, BACKUP_S3_REGION, BACKUP_S3_PATH.
    """
    ...
    try:
        ...
        raise RuntimeError('boto3 is not installed. Add boto3 to requirements.txt') from exc

    region = os.getenv('BACKUP_S3_REGION') or 'auto'
    ...
    return {'bucket': bucket, 'key': key, 'endpoint_url': endpoint_url}
