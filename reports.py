...
def all_clients_report_excel():
    ...
    output = BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        pd.DataFrame(summary_rows).to_excel(writer, index=False, sheet_name='summary_by_clients')
        pd.DataFrame(balance_rows).to_excel(writer, index=False, sheet_name='balances')
