...
def _addendum_name(t):
    addendum_file = getattr(t, 'addendum_file', None)
    if addendum_file:
        return (addendum_file.original_name or addendum_file.title or '').strip() or '— без доп. соглашения —'
...
def _filter_talons_by_client(talons, client_id):
    if not client_id:
        return talons
    return [t for t in talons if int(getattr(t, 'client_id', 0) or 0) == int(client_id)]
...
def client_report_excel(client_id: int):
    ...
    category_label = category_label or 'Все'
...
@reports_bp.get('/reports/addendums')
@login_required
...
@reports_bp.get('/reports/addendums.xlsx')
@login_required
